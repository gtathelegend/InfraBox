      ...
      await loginWithRedirect({
        authorizationParams: {
          ...
          ...(connection === "Username-Password-Authentication"
            ? { screen_hint: "login", login_hint: email || undefined }
            : {}),
        },
      });
    } finally {
        ...
        <div className="hidden flex-col justify-between rounded-3xl border border-white/35 bg-white/35 p-10 backdrop-blur-xl lg:flex">
          <div>
            <div className="mb-6 flex items-center gap-3">
              <InfraboxLogo className="h-11 w-11 rounded-xl shadow-[0_10px_28px_rgba(255,122,24,0.3)]" />
              <div>
            ...
            <h1 className="mt-6 text-4xl font-bold text-slate-900">Predict. Protect. Deploy.</h1>
          ...
          <div className="space-y-4">
            <div className="rounded-2xl bg-white/80 p-4">
              <p className="text-xs uppercase tracking-[0.12em] text-slate-500">Trusted by teams</p>
              <p className="mt-2 text-sm text-slate-700">Kubernetes, Vercel, AWS, and hybrid platforms.</p>
            </div>
            <div className="rounded-2xl bg-white/80 p-4">
              <p className="text-xs uppercase tracking-[0.12em] text-slate-500">Today</p>
              <p className="mt-2 text-sm font-medium text-slate-800">Deployment confidence: 82 / 100</p>
            </div>
          </div>
        </div>
          ...
          >
            <Card className="glass-card rounded-3xl">
              <CardHeader className="space-y-2 text-center">
                <CardTitle className="text-2xl font-bold text-slate-900">Sign in to Infrabox</CardTitle>
                <p className="text-sm text-slate-600">Auth0 secure login with GitHub, Google, or email.</p>
              </CardHeader>
              <CardContent className="space-y-4">
                ...
                <RippleButton
                  ...
                  onClick={() => startLogin("Username-Password-Authentication")}
                  disabled={submitting || isLoading}
                >
                  {submitting || isLoading ? "Redirecting..." : "Continue with Email"}
                </RippleButton>
              </CardContent>
            </Card>
          </motion.div>
