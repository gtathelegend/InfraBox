...
export function WorkspaceProvider({ children }: { children: React.ReactNode }) {
  ...
  React.useEffect(() => {
    ...
    try {
      const parsed = JSON.parse(raw) as Partial<RepoContext>;
      if (
        !parsed ||
        ...
        typeof parsed.lastCommitAgo !== "string"
      ) {
        throw new Error("Invalid cached repository");
      }

      setSelectedRepoState(parsed as RepoContext);
    } catch {
    ...
    async (repositoryId?: string) => {
      ...
      const avgRisk =
        dashboard.predictions.length > 0
          ...
          : 0;
      const derivedInfraHealth = Math.round(
        dashboard.metrics.remediationScore * 0.6 +
          (100 - dashboard.metrics.failurePredictionScore) * 0.4,
      );
      ...
      setMetrics({
        ...
        infrastructureHealth: Math.max(0, Math.min(100, derivedInfraHealth || Math.round(100 - avgRisk * 0.75))),
  ...
  const runFullAnalysis = React.useCallback(
        ...
        toast({
          ...
          description:
            error instanceof Error
              ? error.message
              : "Unable to run analysis for this repository.",
          variant: "destructive",
        });
      } finally {
    ...
    [isAnalyzing, refreshDashboard, selectedRepo?.id, toast],
  );
